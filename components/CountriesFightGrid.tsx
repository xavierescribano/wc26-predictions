      ...
      <div className="p-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 text-center space-y-1.5">
            <p className="text-xs font-semibold text-slate-600 dark:text-slate-300 truncate">{fight.teamAName}</p>
            {fight.isOpen ? (
              <input
                ...
                placeholder="0"
                className="w-full text-center text-2xl font-bold py-3 rounded-xl border bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-600 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
              />
            ) : (
              <div className={`py-3 rounded-xl text-2xl font-bold text-center ${
                hasPick ? "bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white" : "bg-slate-50 dark:bg-slate-800/50 text-slate-400"
              }`}>
                {hasPick ? pick!.goalsA : "–"}
              </div>
            )}
          ...
          {/* Team B */}
          <div className="flex-1 text-center space-y-1.5">
            <p className="text-xs font-semibold text-slate-600 dark:text-slate-300 truncate">{fight.teamBName}</p>
            {fight.isOpen ? (
              <input
                type="number" min="0" max="99"
                value={goalsB}
        ...
        {/* Result row */}
        {hasResult && (
          <div className="mt-3 flex items-center justify-center gap-2 py-2 rounded-lg bg-slate-100 dark:bg-slate-800/60">
            <span className="text-sm font-bold text-slate-900 dark:text-white">
              {fight.teamAName} {fight.resultA} – {fight.resultB} {fight.teamBName}
            </span>
            <span className="text-xs text-slate-500">({outcomeLabel(fight.resultA!, fight.resultB!)})</span>
          </div>
        )}
        ...
        {fight.isOpen && (
          <div className="mt-3 space-y-2">
            ...
            <button
              ...
              className="w-full py-2 rounded-lg text-sm font-semibold transition-colors bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-300 dark:disabled:bg-slate-700 disabled:text-slate-400 disabled:cursor-not-allowed text-white"
            >
    ...
    <div className="space-y-8">
      {open.length > 0 && (
        <div className="space-y-4">
          <h2 className="text-sm font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Open for betting</h2>
      ...
      {closed.length > 0 && (
        <div className="space-y-4">
          <h2 className="text-sm font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Closed</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {closed.map((fight) => (
              <FightCard key={fight.id} fight={fight} pick={picks.find((p) => p.fightId === fight.id)} />
            ))}
