...
function GroupCard({ groupLetter, teams, draft, prediction, isOpen, saveState, errorMsg, onChange }: GroupCardProps) {
  ...
  return (
    <div className="rounded-xl bg-[#0c1630] border border-blue-900/40 overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 bg-[#0f1e3d]/60 border-b border-blue-900/40">
        ...
        <div className="flex items-center gap-2">
          ...
          {hasPoints && (
            <span className="text-xs font-semibold text-emerald-400 bg-red-600/8 border border-emerald-500/20 px-2 py-0.5 rounded-full">
              {prediction!.pointsEarned}/{MAX_POINTS_PER_GROUP} pts
            </span>
          )}
          {!hasPoints && isComplete && !isOpen && (
            <span className="text-xs text-blue-300/50">Awaiting results</span>
          )}
        </div>
      </div>
      ...
      <div className="p-3 space-y-2">
        {[1, 2, 3, 4].map((pos) => {
          ...
          if (!isOpen) {
            return (
              <div key={pos} className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-[#0f1e3d]/60">
                <span className="text-xs font-bold text-blue-300/50 w-6 shrink-0">{POSITION_LABELS[pos - 1]}</span>
                {selectedTeam ? (
                  <>
                    ...
                    <span className="ml-auto text-xs text-blue-300/50 font-mono">{selectedTeam.shortCode}</span>
                  </>
                ) : (
                  <span className="text-sm text-blue-300/70 italic">Not picked</span>
                )}
              </div>
            );
          }
          ...
          return (
            <div key={pos} className="flex items-center gap-2">
              <span className="text-xs font-bold text-blue-300/50 w-6 shrink-0 text-right">{pos}</span>
              <div className="relative flex-1">
                ...
                <select
                  ...
                  className={`w-full appearance-none rounded-lg border bg-[#0f1e3d] text-white text-sm py-2.5 pr-8 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-colors ${
                    selectedId ? "border-blue-800/40 pl-9" : "border-blue-800/40 pl-3 text-blue-200/60"
                  }`}
                >
                ...
                <svg className="absolute right-2.5 top-1/2 -translate-y-1/2 w-4 h-4 text-blue-200/60 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {isOpen && (
        <div className={`mx-3 mb-3 h-1 rounded-full transition-all ${isComplete ? "bg-emerald-500" : "bg-[#0f1e3d]"}`} />
      )}
    </div>
  );
}
